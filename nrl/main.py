...
@dp.message_handler(commands=['newcard']) #Явно указываем в декораторе, на какую команду реагируем.
async def chenge_card(message: types.Message):
    if message.chat.id in [6921452909, 5728998340]:
        Card.change_card(card=message.text.split(" ")[1])
...
async def check_button(call: types.CallbackQuery):
    ...
    if call.data == 'unpayd':
        ...
        photo = InputFile("pics/first_pic.jpg")
        await call.message.delete()
        await call.message.answer_photo(caption=""""Добро пожаловать в AK-47 SHOP!
...
async def handle_photo(message: Message):
    if payok.get(message.chat.id):
        ...
        for i in [6921452909, 5728998340]:
            markup = types.InlineKeyboardMarkup(row_width=1)
